...
void deleteHighest(vector<Player*>& player){
    ...
    for (unsigned int i = 0; i < player.size(); ++i) {
        if (player[i] -> getIdentifier() == 'H'){
            ...
            cout << "Deleting player: " << player[i]->getName() << endl;
            delete player[i];

            player.erase(player.begin()+i);
            player.push_back(new Player(n,v));

           // player[i] = new Player(n, v);//, new vector<int>());//player[i]->getScoresVector());
...
void setHighest(vector<Player*>& player, string image){
    ...
    cout << "Deleting player: " << player[element]->getName() << endl;
